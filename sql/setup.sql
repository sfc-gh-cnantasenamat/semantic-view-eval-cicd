...
-- Lightweight OWNER'S RIGHTS SP that re-grants ownership on both eval result
-- datasets after EXECUTE_AI_EVALUATION resets them. Called from eval_sv.sh and
-- eval.sh after each eval completes, before reading scores.
CREATE OR REPLACE PROCEDURE SV_EVAL_CICD.APP.SP_GRANT_EVAL_OWNERSHIP()
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS OWNER
AS
$$
BEGIN
  GRANT OWNERSHIP ON DATASET SV_EVAL_CICD.APP.GROWTH_ANALYTICS_SYSTEM_EVAL
    TO ROLE SV_EVAL_CICD_ROLE COPY CURRENT GRANTS;
  GRANT OWNERSHIP ON DATASET SV_EVAL_CICD.APP.GROWTH_AGENT_EVAL
    TO ROLE SV_EVAL_CICD_ROLE COPY CURRENT GRANTS;
  RETURN 'Eval ownership granted';
END;
$$;

GRANT USAGE ON PROCEDURE SV_EVAL_CICD.APP.SP_GRANT_EVAL_OWNERSHIP()
  TO ROLE SV_EVAL_CICD_ROLE;
